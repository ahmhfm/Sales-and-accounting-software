...
namespace Mesrakh
{
    public partial class frmBarcode : Form
    {
        ...
        public void fillTblBarcode()// تعبئة جدول الباركودات
        {
            try
            {
                if (TblBarcode == null) { TblBarcode = new DataTable(); } // جدول الباركورد
                ...
                if (this.Parent.Parent.Controls["txt08001002001"] != null)
                {
                    ...
                    {
                        TblBarcode = DataTools.DataBases.ReadTabel("select * from TblBarcode where UnitDetailsNo = " + this.Parent.Parent.Controls["txt08001002001"].Text.Trim());
                    }
                }
        ...
        private void txt002EventsAndProperties(bool Properties, bool Events = false) // مربع نص رقم الباركود 
        {
            if (Properties == true && Events == false)
            {
                ElementsTools.TextBox_.CustumProperties(txt002, "",20, ColorPropertyName.BackColor_1, ColorPropertyName.ForeColor_1);

            }
            else if (Properties == true && Events == true)
            {
                ElementsTools.TextBox_.CustumProperties(txt002, "",20, ColorPropertyName.BackColor_1, ColorPropertyName.ForeColor_1, true, true);
                    ...
                    try
                    {
                        if (btn001.Text=="إلغاء")
                        {
                            ...
                            if (Error == "")
                            {
                                ...
                                string[] result = null;
                                DataTools.DataBases.Run(ref result, "insert into TblBarcode values (@UnitDetailsNo,@BarcodeNo);"
                        ...
                        else if(btn003.Text=="إلغاء")
                        {
                            ...
                            if (Error == "")
                            {
                                ...
                                DataTools.DataBases.Run(ref result, "update TblBarcode set BarcodeNo = @BarcodeNo where BarcodeSN = @BarcodeSN"
                                    , CommandType.Text, new object[][] {
                                          new object[] { "@UnitDetailsNo", this.Parent.Parent.Controls["txt08001002001"].Text.Trim() == "" }
                                        , new object[] { "@BarcodeNo", txt002.Text.Replace("'", "").Trim() }
