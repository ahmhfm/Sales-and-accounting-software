    ...
    public class DataBases
    {
        ...
        public static object Run(ref string[] result ,string CommandString, CommandType commandtype, params object[][] ParameterAndValue)
        {
            ...
            catch (Exception ex)
            {
                ...
                // 94
                else if(ex.Message.Contains("pk_TblBankAccounts_TblEnterpriseBranches_BranchNumber"))
                {
                    string ArabicMessage = "عدم توافق مع سجلات جدول الحسابات البنكية";
                    frmMessageBox.ShowMessageBox(Cultures.ReturnTranslation("تنبيه"), ArabicMessage, MessageBoxStatus.Ok);
                    GeneralAction.AddNewNotification("DataTools >> DataBases >> Run ", DateTime.Now, Cultures.ReturnTranslation(ArabicMessage), ex.Message, result[2], result[4]);
                }
                //95
                else if (ex.Message.Contains("pk_TblCashBoxes_TblEnterpriseBranches_BranchNumber"))
                {
                    string ArabicMessage = "عدم توافق مع سجلات جدول صناديق النقدية";
                    frmMessageBox.ShowMessageBox(Cultures.ReturnTranslation("تنبيه"), ArabicMessage, MessageBoxStatus.Ok);
                    GeneralAction.AddNewNotification("DataTools >> DataBases >> Run ", DateTime.Now, Cultures.ReturnTranslation(ArabicMessage), ex.Message, result[2], result[4]);
                }
                //96
                ...
                {
                    string ArabicMessage = "عدم توافق مع سجلات جدول الارفف";
                    frmMessageBox.ShowMessageBox(Cultures.ReturnTranslation("تنبيه"), ArabicMessage, MessageBoxStatus.Ok);
                    GeneralAction.AddNewNotification("DataTools >> DataBases >> Run ", DateTime.Now, Cultures.ReturnTranslation(ArabicMessage), ex.Message, result[2], result[4]);
                }
                //97
                ...
                {
                    string ArabicMessage = "عدم توافق مع سجلات جدول وحدات العمليات";
                    frmMessageBox.ShowMessageBox(Cultures.ReturnTranslation("تنبيه"), ArabicMessage, MessageBoxStatus.Ok);
                    GeneralAction.AddNewNotification("DataTools >> DataBases >> Run ", DateTime.Now, Cultures.ReturnTranslation(ArabicMessage), ex.Message, result[2], result[4]);
                }
                //  95
        ...
        // قراءة جدول في قاعدة البيانات واعادة جميع الاعمدة منه
        /// <summary>
