...
public class GeneralAction
{
    ...
    public static void RefreshAllElementsProperties()
    {
        ...
        if (GeneralVariables._frmSales23 != null) { GeneralVariables._frmSales23.AllEventsAndProperties(true); } // تحديث خصائص نموذج المبيعات  
        if (GeneralVariables._frmStandardAccounts24 != null) { GeneralVariables._frmStandardAccounts24.AllEventsAndProperties(true); } // تحديث خصائص نموذج الحسابات المعيارية  
        if (GeneralVariables._frmInventoryOperations25 != null) { GeneralVariables._frmInventoryOperations25.AllEventsAndProperties(true); } // تحديث خصائص نموذج عمليات المخزون  

    }

    // اغلاق جميع النماذج وذلك بسبب الخروج من الحساب
    public static void CloseAllForms()
    {
        GeneralVariables._frmConnectionSettings = null;
        GeneralVariables._frmAccount05 = null;
        GeneralVariables._frmMainMenu06 = null;
    ...
    /// اعادة بيانات العميل
    /// </summary>
    /// <param name="ClientsNo"></param>
    public static string[] returnClientData(string ClientsNo)
    {
        string[] result = new string[3];
        if(!string.IsNullOrEmpty(ClientsNo))
        {
            var v = from emp in GeneralVariables.TblClients.AsEnumerable()
                    where emp[0].ToString().Contains(ClientsNo)
                    select emp;
            foreach (var item in v)
            {
                result[0] = item[1].ToString();
                result[1] = item[2].ToString();
                result[2] = item[19].ToString();
            }
        }
