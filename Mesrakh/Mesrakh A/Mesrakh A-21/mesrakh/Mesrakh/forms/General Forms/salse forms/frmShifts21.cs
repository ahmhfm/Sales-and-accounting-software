...
namespace Mesrakh
{
    public partial class frmShifts21 : Form
    {
        ...
        {
            CheckOpenShiftsOnThisDevice(); //// فحص الورديات المفتوحة على هذا الجهاز
            OpenShiftsForThisUserAccountOnOtherDevices(); // فحص الورديات المفتوحة لحساب المستخدم هذا على الاجهزة الاخرى
            ...
            fillTblOpenShifts();  // تعبئة الورديات المفتوحة لجميع الحسابات
        ...
        private void cbxShiftsForClosingEventsAndProperties(bool Properties, bool Events = false) // قائمة منسدلة ورديات للإغلاق
        {
            ...
            if (Properties == true && Events == false)
            {
                try
                {
                    if (TblShiftsForClosing is null) { TblShiftsForClosing = new DataTable(); fillTblShiftsForClosing(); }
                    if (TblShiftsForClosing.Columns.Count > 1)
                    {
                ...
                btn001002002.Click += delegate
                {
                    try
                    {
                        ...
                        if (btn001002001.Text==Cultures.ReturnTranslation("إلغاء"))
                        {
                            ...
                            string ReceivedShiftNumber = cbxShiftsForClosing.SelectedIndex < 0 ? "Null" : cbxShiftsForClosing.SelectedValue.ToString() ;
                            ...
                            string ClosingUserAccountNo = cbxRecevingAccount.SelectedIndex < 0 ? "Null" : cbxRecevingAccount.SelectedValue.ToString() ;
                            ...
                            if(!string.IsNullOrEmpty(ReceivedShiftNumber))
                            {
...
private void dgv001EventsAndProperties(bool Properties, bool Events = false) // جدول عرض الورديات المغلقة لهذا الحساب 
        ...
        DataTable TblOpenShiftsForThisOperationUnit = new DataTable(); // الوردية المفتوحة لوحدة العمليات الحالية 
        ...
        private void fillTblShiftsForClosing() // تعبئة جدول ورديات للإغلاق
        {
            try
            {
                ...
                if (TblShiftsForClosing.Rows.Count > 0) TblShiftsForClosing.Rows.Clear();
        ...
        private void OpenShiftsForThisUserAccountOnOtherDevices() // فحص الورديات المفتوحة لحساب المستخدم هذا على الاجهزة الاخرى
        {
            try
            {
                ...
                TblOpenShiftsForThisUserAccountOnOtherDevices = DataTools.DataBases.ReadTabel(@"select * from TblShifts inner join TblOperationUnits on TblShifts.OperationUnitNumber = TblOperationUnits.OperationUnitNumber where DeviceNumber != '" + SecurityTools.specialDevices.getCPUID() + "' and DateAndTimeOfShiftClosing  = '1999-01-01 00:00:00.000' ");
                ...
                if (TblOpenShiftsForThisUserAccountOnOtherDevices != null)
                {
                    if (TblOpenShiftsForThisUserAccountOnOtherDevices.Rows.Count > 0)
                    {
                            ...
                            frmMessageBox.ShowMessageBox(Cultures.ReturnTranslation("تنبيه"), Cultures.ReturnTranslation( "رجاء قم بإغلاق الوردية المفتوحة على وحدة العمليات رقم")+"\n"+ TblOpenShiftsForThisUserAccountOnOtherDevices.Rows[0][14], MessageBoxStatus.Ok);
                            foreach (Control control in this.Controls)
                            {
                                control.Enabled = false;
                            }
        ...
        //------------------------------------------------------
        private void fillDataForAllElements()
        {
            if (TblOpenShiftsForThisAccount.Rows.Count > 0)
            {
                ...
                cbxShiftsForClosing.Text = TblOpenShiftsForThisAccount.Rows[0][2].ToString();//-- رقم الوردية المستلمة
                ...
                cbxRecevingAccount.SelectedValue = TblOpenShiftsForThisAccount.Rows[0][8].ToString() == "" ? -1 : TblOpenShiftsForThisAccount.Rows[0][8];//-- رقم الحساب الذي قام بالاقفال
                txt001002007.Text = TblOpenShiftsForThisAccount.Rows[0][9].ToString();//--الرصيد القيدي
                txt001002008.Text = TblOpenShiftsForThisAccount.Rows[0][10].ToString();//--الرصيد الفعلي
                txt001002009.Text = TblOpenShiftsForThisAccount.Rows[0][11].ToString();//--الفرق بين الرصيدين
