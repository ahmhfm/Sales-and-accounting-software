        ...
        public void fillTblFreeUnitsOffers()// تعبئة جدول عروض الوحدات المجانية
        {
            try
            {
                ...
                if (this.Parent.Parent.Controls["txt08001002001"] != null)
                {
                    ...
                    {
                        TblFreeUnitsOffers = DataTools.DataBases.ReadTabel("select * from TblFreeUnitsOffers where MeasureAndEncapsulationUnitNo = " + this.Parent.Parent.Controls["txt08001002001"].Text.Trim());
                    }
                }
                    ...
                    try
                    {
                        if (btn001.Text == "إلغاء")
                        {
                            ...
                            if (Error == "")
                            {
                                ...
                                string[] result = null;
                                DataTools.DataBases.Run(ref result, "insert into TblFreeUnitsOffers values (@MeasureAndEncapsulationUnitNo,@LessQuantity, @HighestQuantity, @NumberOfFreeUnits ,@PromoStartDate,@PromoEndDate );"
