...
namespace Mesrakh
{
    public partial class frmEnterpriseBranches16 : Form
    {
                    ...
                    try
                    {
                        if (Permissions.AccountHavePermission(PermissionObjectes.Unit_Branch, PermissionType.Add))
                        {
                            ...
                            else
                            {
                                ...
                                if (txt08001002001.Text.Trim() == "")
                                {
                                    check += Cultures.ReturnTranslation("رجاء قم بتحديد الفرع الذي تريد إضافة طريقة إتصال جديدة إليه");
        ...
        DataTable TblOperationUnits = new DataTable(); // جدول بيانات طرق الاتصال بالشركة الصانعة المحددة
