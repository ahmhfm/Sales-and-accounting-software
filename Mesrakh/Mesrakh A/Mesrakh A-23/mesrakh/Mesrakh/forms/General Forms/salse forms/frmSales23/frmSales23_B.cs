...
namespace Mesrakh
{
    public partial class frmSales23_B : Form
    {
            ...
            else if (Properties == true && Events == true)
            {
                ...
                btn004.Click += delegate
                {
                    ...
                    new object[][] {
                                    ...
                                    , new object[] { "@ShiftNumber", (this.Parent.Parent.Parent.Parent.Parent as frmSales23).TblOpenShiftsForThisOperationUnit.Rows[0][0].ToString() } //  رقم الوردية
                    ...
                    if (result[1] == "succeeded")
                    {
                        ...
                        frm.ShiftNumber = TblShifts.Rows[0][0].ToString(); // رقم الوردية
                        frm.BalanceInRecords = TblShifts.Rows[0][9].ToString(); // الرصيد القيدي بجدول الورديات
                        frm.OperationUnitNumber = TblShifts.Rows[0][5].ToString(); // رقم وحدة العمليات
                        
                        frm.OperationUnitAccountNo = currentOperationAccountNo()[1].ToString(); // رقم حساب وحدة العمليات

                        frm.BranchNumber = currentOperationAccountNo()[0].ToString(); // رقم الفرع
        ...
        public DataTable TblShifts = new DataTable(); // الوردية المفتوحة  
        ...
        private void fillTblShifts() // تعبئة بيانات جدول الوردية المفتوحة
        {
            try
            {
                ...
                TblShifts = DataTools.DataBases.ReadTabel(@"select * from TblShifts where  ShiftNumber = " + (this.Parent.Parent.Parent.Parent.Parent as frmSales23).TblOpenShiftsForThisOperationUnit.Rows[0][0].ToString() );
