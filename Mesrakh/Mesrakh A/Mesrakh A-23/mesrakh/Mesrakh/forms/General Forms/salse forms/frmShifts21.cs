        ...
        private void frmEventsAndProperties(bool Properties, bool Events = false) // النموذج 
        {
            ...
            else if (Properties == true & Events == true)
            {
                ...
                this.Load += delegate
                {
                    ...
                    CheckOpenShiftsOnThisDevice();  //  الورديات المفتوحة على هذا الجهاز
                    ...
                    {
                        if(TblOpenShiftsForThisOperationUnit.Rows[0][4].ToString() == CurrentAccountNo)// نعم الوردية المفتوحة على هذا الجهاز تخص المستخدم الحالي
                        {
                            fillDataForAllElements(TblOpenShiftsForThisOperationUnit); // تعبئة جميع العناصر ببيانات الشفت
                        }
                        else// لا الوردية المفتوحة على هذا الجهاز لا تخص المستخدم الحالي
                        {
                            if(TblOpenShiftsForThisOperationUnit.Rows[0][6].ToString() == "2") // نعم حالية الوردية المفتوحة على هذا الجهاز للإغلاق
                            {
                                ...
                                if (GeneralVariables.MessageBoxResult == "Yes")// اجراءات استلام الوردية الموجودة على هذا الجهاز
                                {
                                    ...
                                    cbxShiftsForClosing.SelectedValue = TblOpenShiftsForThisOperationUnit.Rows[0][0].ToString();// الوردية المستلمة
...
private void dgv001EventsAndProperties(bool Properties, bool Events = false) // جدول عرض الورديات المغلقة لهذا الحساب 
        ...
        private void CheckOpenShiftsOnThisDevice() //  الورديات المفتوحة على هذا الجهاز
        {
            try
            {
                ...
                TblOpenShiftsForThisOperationUnit = DataTools.DataBases.ReadTabel(@"select * from TblShifts inner join TblOperationUnits on TblShifts.OperationUnitNumber = TblOperationUnits.OperationUnitNumber where DeviceNumber = '"+ SecurityTools.specialDevices.getCPUID() + "' and ShiftStatus  in (1,2) ");
                ...
                //        return TblOpenShiftsForThisOperationUnit;
                ...
                //        //    if(TblOpenShiftsForThisOperationUnit.Rows[0][6].ToString() == "2")
                //        //    {
                //        //        frmMessageBox.ShowMessageBox(Cultures.ReturnTranslation("تنبيه"), Cultures.ReturnTranslation("هل تريد إستلام الوردية الموجودة على هذا الجهاز"), MessageBoxStatus.YesAndNo);
