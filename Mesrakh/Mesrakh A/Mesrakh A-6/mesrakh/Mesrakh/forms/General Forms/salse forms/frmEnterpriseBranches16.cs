...
namespace Mesrakh
{
    public partial class frmEnterpriseBranches16 : Form
    {
        ...
        private void lbl08001002006EventsAndProperties(bool Properties, bool Events = false) // ملصق العنوان 
        {

            if (Properties == true && Events == false)
            {
                ElementsTools.Lable_.CustumProperties(lbl08001002006, "العنوان", ColorPropertyName.BackColor_1);
            }
            else if (Properties == true && Events == true)
            {
                ElementsTools.Lable_.CustumProperties(lbl08001002006, "العنوان", ColorPropertyName.BackColor_1, ColorPropertyName.ForeColor_1, true, true);
            }
        }
        ...
        private void txt08001002004EventsAndProperties(bool Properties, bool Events = false) // مربع نص العنوان
        {
            ...
            if (Properties == true && Events == false)
            {
                ElementsTools.TextBox_.CustumProperties(txt08001002004, cultureInfo);
            }
            else if (Properties == true && Events == true)
            {
                ElementsTools.TextBox_.CustumProperties(txt08001002004, cultureInfo, ColorPropertyName.BackColor_1, ColorPropertyName.ForeColor_1, true, true);
        ...
        private void txt08001002005EventsAndProperties(bool Properties, bool Events = false) // مربع نص التفاصيل
        {
            ...
            if (Properties == true && Events == false)
            {
                ElementsTools.TextBox_.CustumProperties(txt08001002005, cultureInfo);
            }
            else if (Properties == true && Events == true)
            {
                ElementsTools.TextBox_.CustumProperties(txt08001002005, cultureInfo, ColorPropertyName.BackColor_1, ColorPropertyName.ForeColor_1, true, true);
                    ...
                    try
                    {
                        ...
                        if (txt08001002001.Text.Trim() == "")
                        {
                            ...
                            if (Error == "")
                            {
                                ...
                                object BranchNumber = DataTools.DataBases.Run(ref result00, "spAddEnterpriseBranches", CommandType.StoredProcedure, new object[][] { 
                                    ...
                                    new object[] { "@TheAddress", txt08001002004.Text },
                        ...
                        else
                        {
                            ...
                            if (Error == "")
                            {
                                ...
                                string[] result00 = null;
                                object result2 = DataTools.DataBases.Run(ref result00, "update TblEnterpriseBranches set BranchTypeNo = @BranchTypeNo,BranchNameAR = @BranchNameAR ,BranchNameEN = @BranchNameEN ,TheAddress = @TheAddress ,BranchDetail = @BranchDetail where BranchNumber = @BranchNumber ", CommandType.Text, new object[][] {
        ...
        private void txtUnitNoInBranchEventsAndProperties(bool Properties = true, bool Events = false) // مربع نص رمز وحدة العمليات في الفرع
        {
            ...
            if (Properties == true && Events == false)
            {
                ElementsTools.TextBox_.CustumProperties(txtUnitNoInBranch, cultureInfo);
            }
            else if (Properties == true && Events == true)
            {
                ElementsTools.TextBox_.CustumProperties(txtUnitNoInBranch, cultureInfo, ColorPropertyName.BackColor_1, ColorPropertyName.ForeColor_1, true, true);
        ...
        private void lblShelfGeneralNoEventsAndProperties(bool Properties, bool Events = false) // ملصق الرقم المتسلسل للرف
        {

            if (Properties == true & Events == false)
            {
                ElementsTools.Lable_.CustumProperties(lblShelfGeneralNo, "الرقم المتسلسل للرف", ColorPropertyName.BackColor_1);
            }
            else if (Properties == true & Events == true)
            {
                ElementsTools.Lable_.CustumProperties(lblShelfGeneralNo, "الرقم المتسلسل للرف", ColorPropertyName.BackColor_1, ColorPropertyName.ForeColor_1, true, true);
            }
        }


        private void txtShelfGeneralNoEventsAndProperties(bool Properties = true, bool Events = false) // مربع نص الرقم المتسلسل للرف
        {
            string cultureInfo = "en-US";
            if (Properties == true && Events == false)
            {
                ElementsTools.TextBox_.CustumProperties(txtShelfGeneralNo, cultureInfo);
            }
            else if (Properties == true && Events == true)
            {
                ElementsTools.TextBox_.CustumProperties(txtShelfGeneralNo, cultureInfo, ColorPropertyName.BackColor_1, ColorPropertyName.ForeColor_1, true, true);

                txtShelfGeneralNo.KeyDown += delegate (object sender, KeyEventArgs e)
                {
                    ...
                    try
                    {
                        if (Permissions.AccountHavePermission(PermissionObjectes.Shelves, PermissionType.Add))
                        {
                            ...
                            if (btnAddShelf.Text == Cultures.ReturnTranslation("إضافة"))
                            {
                                ...
                                txtShelfGeneralNo.Enabled = true;
                                txtShelfGeneralNo.Text = "";
                                dgvShelves.Enabled = true;

                                txtShelfGeneralNo.Select();
                            }
                            else
                            {
                                string check = "";
                                ...
                                else if (txtShelfGeneralNo.Text.Trim() == "")
                                {
                                    check += Cultures.ReturnTranslation("رجاء قم بتسجيل رمز الرف الجديد");
                                    txtShelfGeneralNo.Focus();
                                }
                                else
                                {
                                    bool? SameShelfCode = ElementsTools.DataGridView_.findValueThenSelected(dgvShelves, ActionType.add, txtShelfGeneralNo.Text.Replace("'", ""), 1);
                                ...
                                {
                                    btnAddShelf.Text = Cultures.ReturnTranslation("إضافة");
                                    txtShelfGeneralNo.Enabled = false;

                                    //----------

                                    string[] result = null;
                                    object x = DataTools.DataBases.Run(ref result, "insert into TblShelves values (@ShelfCode,@BranchNumber)", CommandType.Text, new object[][] { new object[] { "@ShelfCode", txtShelfGeneralNo.Text }  ,  new object[] { "@BranchNumber", txt08001002001.Text }});
                ...
                dgvShelves.SelectionChanged += delegate
                {
                    try
                    {
                        if (dgvShelves.SelectedRows != null)
                        {
                            if (dgvShelves.SelectedRows.Count > 0)
                            {
                                txtShelfGeneralNo.Text = dgvShelves.SelectedRows[0].Cells[1].Value.ToString();
                            }
                        }
