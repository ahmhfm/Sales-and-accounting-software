...
public enum MessageBoxStatus { Ok, YesAndNo, Text , Number }
...
public partial class frmMessageBox : Form
{
    ...
    public void AllEventsAndProperties(bool Properties, bool Events = false)  //  جميع العناصر
    {
        ...
        txt0501EventsAndProperties(Properties, Events); // مربع استلام القيم
    }
    ...
    private void frmMessageBoxEventsAndProperties(bool Properties, bool Events = false) //   النموذج 
    {
        ...
        if (Events)
        {
            this.Load += delegate
            {
                ...
                else if (messageBoxState == MessageBoxStatus.Number)
                {
                    pnl03.Visible = false;
                    pnl04.Visible = false;
                    pnl05.Visible = true;
    ...
    private void txt0501EventsAndProperties(bool Properties, bool Events = false) // مربع نص استلام القيم
    {
        if (Properties == true & Events == false)
        {
            if(messageBoxState == MessageBoxStatus.Text)
            {
                ElementsTools.TextBox_.CustumProperties(txt0501, "", 300, ColorPropertyName.BackColor_2, ColorPropertyName.ForeColor_1,true,false,TextBoxType.String);
            }
            else if(messageBoxState == MessageBoxStatus.Number)
            {
                ElementsTools.TextBox_.CustumProperties(txt0501, "", 10, ColorPropertyName.BackColor_2, ColorPropertyName.ForeColor_1, true, false, TextBoxType.Decimal);
            }
        }
        else if (Properties == true & Events == true)
        {
            ...
            if (messageBoxState == MessageBoxStatus.Text)
            {
                ElementsTools.TextBox_.CustumProperties(txt0501, "", 300, ColorPropertyName.BackColor_2, ColorPropertyName.ForeColor_1, true, true, TextBoxType.String);
            }
            else if (messageBoxState == MessageBoxStatus.Number)
            {
                ElementsTools.TextBox_.CustumProperties(txt0501, "", 10, ColorPropertyName.BackColor_2, ColorPropertyName.ForeColor_1, true, true, TextBoxType.Decimal);
            }

            txt0501.KeyDown += delegate (object mysender, KeyEventArgs mye)
            {
                if (mye.KeyData == Keys.Enter)
                {
                    btn0502.PerformClick();
                }
            };
        }
    }
