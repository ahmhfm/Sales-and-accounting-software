        ...
        public void fillTblPriceCategories()// تعبئة جدول الفئات السعرية
        {
            try
            {
                ...
                if (this.Parent.Parent.Controls["txt08001002001"] != null)
                {
                    ...
                    {
                        TblPriceCategories = DataTools.DataBases.ReadTabel("select * from TblPriceCategories where UnitDetailsNo = " + this.Parent.Parent.Controls["txt08001002001"].Text.Trim());
                    }
                }
                    ...
                    try
                    {
                        if (btn001.Text == "إلغاء")
                        {
                            ...
                            if (Error == "")
                            {
                                ...
                                string[] result = null;
                                DataTools.DataBases.Run(ref result, "insert into TblPriceCategories values (@UnitDetailsNo,@LessQuantity, @HighestQuantity, @Price);"
