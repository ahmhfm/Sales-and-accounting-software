...
namespace Mesrakh
{
    public partial class frmSales23 : Form
    {
            ...
            try
            {
                ...
                if (TblOpenShiftsForThisOperationUnit != null)
                {
                    if (TblOpenShiftsForThisOperationUnit.Rows.Count > 0)
                    {
                        ...
                        {
                            fillTblOperationUnits(TblOpenShiftsForThisOperationUnit.Rows[0][5].ToString()); // تحميل بيانات وحدة العمليات
                        }
                    }
        ...
        private void fillTblOperationUnits(string OperationUnitNumber)
        {
            try
            {
                ...
                if (OperationUnitNumber.Trim().Replace("'", "") != "")
                {
                    TblOperationUnits = DataTools.DataBases.ReadTabel("select * from TblOperationUnits where OperationUnitNumber = " + OperationUnitNumber);
                }
