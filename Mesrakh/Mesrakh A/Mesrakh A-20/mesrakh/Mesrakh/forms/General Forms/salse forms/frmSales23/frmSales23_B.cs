    ...
    public partial class frmSales23_B : Form
    {
            ...
            else if (Properties == true && Events == true)
            {
                ...
                btn004.Click += delegate
                {
                    if(lbl0010.Text.Trim() == "")
                    {
                        if(dgv001.Rows.Count > 0)
                        {
                            frmSalseInvoice frm = new frmSalseInvoice();
                            frm.Font = this.Font;
                            frm.Controls["pnlMain"].Controls["txt001"].Text = lbl0088.Text;
                            frm.Controls["pnlMain"].Controls["txt002"].Text = lbl0088.Text;
                            frm.Controls["pnlMain"].Controls["txt003"].Text = "0";
                            frm.SalesCartNo = lbl001.Text;// رقم سلة المشتريات
                            frm.ShiftNumber = (this.Parent.Parent.Parent.Parent.Parent as frmSales23).TblOpenShiftsForThisOperationUnit.Rows[0][0].ToString(); // رقم الوردية
                            frm.ShowDialog();
                        }
                        else
                        {
                            frmMessageBox.ShowMessageBox(Cultures.ReturnTranslation("تنبيه"), Cultures.ReturnTranslation("لا يمكن إصدار فاتورة لعربة تسوق فارغة"), MessageBoxStatus.Ok);

                        }
                    }
                    else
                    {
                        frmMessageBox.ShowMessageBox(Cultures.ReturnTranslation("تنبيه"), Cultures.ReturnTranslation("لا يمكن إصدار أكثر من فاتورة لعربة تسوق واحدة"), MessageBoxStatus.Ok);
        ...
        private void lbl0010EventsAndProperties(bool Properties, bool Events = false) // ملصق رقم الفاتورة 
        {
            ...
            if (Properties == true & Events == false)
            {
                ...
                ElementsTools.Lable_.CustumProperties(lbl0010, "", ColorPropertyName.BackColor_6 , ColorPropertyName.ForeColor_6, true, false);
                lbl0010.BackColor = Color.Transparent;
            }
            else if (Properties == true & Events == true)
            {

                ElementsTools.Lable_.CustumProperties(lbl0010, "", ColorPropertyName.BackColor_6, ColorPropertyName.ForeColor_6, true, true);
                lbl0010.BackColor = Color.Transparent;
            }
        }
        ...
        private void lbl0011EventsAndProperties(bool Properties, bool Events = false) // ملصق تاريخ اصدار الفاتورة 
        {
            ...
            if (Properties == true & Events == false)
            {
                ElementsTools.Lable_.CustumProperties(lbl0011, "", ColorPropertyName.BackColor_6, ColorPropertyName.ForeColor_6, true, false);
                lbl0011.BackColor = Color.Transparent;
            }
            else if (Properties == true & Events == true)
            {
                ElementsTools.Lable_.CustumProperties(lbl0011, "", ColorPropertyName.BackColor_6, ColorPropertyName.ForeColor_6, true, true);
                lbl0011.BackColor = Color.Transparent;
            }
        }
